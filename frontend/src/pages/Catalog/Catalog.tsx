...
const Catalog: React.FC = () => {
  ...
  const loadData = async () => {
    ...
    try {
      ...
      if (categorySlug) {
        ...
        console.log('🎯 Current category:', category?.name, '(slug:', categorySlug, ')')
  ...
  const getFilteredProducts = () => {
    ...
    if (currentCategory) {
      ...
      filtered = filtered.filter(p => allowedCategories.includes(p.category.id))
    }
      ...
      <div className="cat-catalog-wrapper">
        <div className="cat-catalog-page">
          <div className="cat-breadcrumbs">
            ...
            <Link 
              to="/catalog" 
              className={`cat-breadcrumb-item ${!currentCategory && !searchQuery ? 'active' : ''}`}
            >
              Каталог
            </Link>
            {searchQuery && !currentCategory && (
              <>
                <span className="cat-breadcrumb-separator">/</span>
                <span className="cat-breadcrumb-item active">Поиск</span>
              </>
            )}
            {breadcrumbs.map((cat, index) => (
              <React.Fragment key={cat.id}>
                ...
                <Link
                  to={`/catalog/${cat.slug}`}
                  className={`cat-breadcrumb-item ${index === breadcrumbs.length - 1 && !searchQuery ? 'active' : ''}`}
                >
                  {cat.name}
                </Link>
              </React.Fragment>
