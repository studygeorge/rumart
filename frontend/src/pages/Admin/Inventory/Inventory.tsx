...
import HeadphonesVariantForm, { type HeadphonesVariant } from './components/VariantForms/HeadphonesVariantForm'
...
type ProductType = 'iphone' | 'ipad' | 'macbook' | 'headphones' | 'generic'
type AnyVariant = iPhoneVariant | iPadVariant | MacBookVariant | HeadphonesVariant | GenericVariant
...
const Inventory: React.FC = () => {
  ...
  const getCategoryHierarchy = (categoryId: string): string[] => {
    ...
    return [...getCategoryHierarchy(category.parentId), category.id]
  }
  ...
  const detectProductType = (categoryId: string): ProductType => {
    ...
    for (const catId of hierarchy) {
      ...
      if (categoryName.includes('iphone') || categorySlug.includes('iphone')) return 'iphone'
      if (categoryName.includes('ipad') || categorySlug.includes('ipad')) return 'ipad'
      if (categoryName.includes('macbook') || categorySlug.includes('macbook') || categoryName.includes('mac') || categorySlug.includes('mac')) return 'macbook'
      if (categoryName.includes('headphones') || categorySlug.includes('headphones') || categoryName.includes('наушники') || categorySlug.includes('naushniki')) return 'headphones'
    }
  ...
  const handleOpenModal = (product?: AdminProduct) => {
    if (product) {
      ...
      if (product.variants && product.variants.length > 0) {
        ...
        console.log('📥 Загруженные варианты для редактирования:', mappedVariants)
        console.log('🔍 Определённый тип продукта:', detectedType)
  ...
  const handleCopyProduct = (product: AdminProduct) => {
    ...
    if (product.variants && product.variants.length > 0) {
      ...
      console.log('📋 Скопированные варианты:', mappedVariants)
      console.log('🔍 Определённый тип продукта:', detectedType)
      setVariants(mappedVariants)
    } else {
      setVariants([])
    ...
    try {
      const cleanedVariants = variants.map(v => {
        ...
        if (productType === 'iphone') {
          ...
          baseVariant.gpu = macBookV.gpu || undefined
        } else if (productType === 'headphones') {
          const headphonesV = v as HeadphonesVariant
          baseVariant.color = headphonesV.color || undefined
          baseVariant.colorHex = headphonesV.colorHex || undefined
          baseVariant.connectivity = headphonesV.connectivity || undefined
        }

        Object.keys(baseVariant).forEach(key => {
          if (baseVariant[key] === undefined) {
            delete baseVariant[key]
          }
        })

        console.log('📦 Отправляемый вариант:', baseVariant)
  ...
  const renderVariantForm = () => {
    ...
    switch (productType) {
      ...
      case 'headphones':
        return React.createElement(HeadphonesVariantForm, commonProps)
      default:
  ...
  const renderCategoryCascade = () => {
    if (categories.length === 0) return <p>Загрузка категорий...</p>

    const hierarchy = formData.categoryId ? getCategoryHierarchy(formData.categoryId) : []
    
    const renderLevel = (level: number, currentPath: string[]): JSX.Element | null => {
      const parentId = level === 0 ? null : currentPath[level - 1]
      const currentLevelCategories = parentId 
        ? categories.filter(cat => cat.parentId === parentId)
        : categories.filter(cat => !cat.parentId)

      if (level > 0 && currentLevelCategories.length === 0) return null
            ...
            onChange={(e) => {
              ...
              if (!newValue) {
                if (level === 0) {
                  ...
                  handleCategoryChange(currentPath[level - 1] || '')
                }
              } else {
                handleCategoryChange(newValue)
              }
            }}
            required={level === 0 && !formData.categoryId}
          >
            <option value="">
              {level === 0 ? 'Выберите категорию' : `Выберите уровень ${level + 1}`}
            </option>
            {currentLevelCategories.map(cat => (
              <option key={cat.id} value={cat.id}>{cat.name}</option>
            ))}
          </select>
          {selectedValue && getChildCategories(selectedValue).length > 0 && renderLevel(level + 1, hierarchy)}
        </React.Fragment>
      )
    }
    ...
    return (
      <div className="category-cascade">
        {renderLevel(0, hierarchy)}
        {formData.categoryId && (
          <p className="category-help-text">
            Выбрано: {hierarchy.map(id => categories.find(c => c.id === id)?.name).filter(Boolean).join(' → ')}
          </p>
        )}
      </div>
    )
              ...
              <div className="form-group">
                ...
                {editingProduct ? (
                  <div className="product-type-display">
                    <span className="type-badge">{getProductTypeLabel(productType)}</span>
                    <span className="type-hint">Тип продукта определяется автоматически по категории</span>
                  </div>
                ) : (
                  <div className="product-type-selector">
                    ...
                    <button
                      type="button"
                      className={`type-btn ${productType === 'headphones' ? 'active' : ''}`}
                      onClick={() => handleProductTypeChange('headphones')}
                    >
                      Headphones
                    </button>
                    <button
