...
const SmartphonesMenu: React.FC = () => {
      ...
      promos: [
        {
          ...
          image: '/images/megamenubanner/smartphones/flagship.jpg',
          alt: 'Флагманы 2025'
        },
        {
          title: 'Гаджеты 2025',
          subtitle: 'Стиль и технологии',
          image: '/images/megamenubanner/smartphones/gadgets.jpg',
          alt: 'Гаджеты 2025'
        }
      ]
    },
  ...
  return (
    <div className="smartphonesmenu-wrapper">
      {/* Левая часть - основные категории */}
      <div className="smartphonesmenu-sidebar">
        <div className="smartphonesmenu-main-category smartphonesmenu-main-category-main">
          <span>Смартфоны и гаджеты</span>
        </div>
        
        <div 
          className={`smartphonesmenu-main-category ${activeCategory === 'samsung' ? 'smartphonesmenu-main-category-active' : ''}`}
        ...
        <div 
          className={`smartphonesmenu-main-category ${activeCategory === 'xiaomi' ? 'smartphonesmenu-main-category-active' : ''}`}
        ...
        <div 
          className={`smartphonesmenu-main-category ${activeCategory === 'google' ? 'smartphonesmenu-main-category-active' : ''}`}
        ...
        <div 
          className={`smartphonesmenu-main-category ${activeCategory === 'other' ? 'smartphonesmenu-main-category-active' : ''}`}
        ...
        <div 
          className={`smartphonesmenu-main-category ${activeCategory === 'gadgets' ? 'smartphonesmenu-main-category-active' : ''}`}
      ...
      <div className="smartphonesmenu-main-content">
        {/* Контент подкатегорий */}
        <div className="smartphonesmenu-content-wrapper">
          <div className="smartphonesmenu-section">
            <h3 className="smartphonesmenu-section-title">{currentContent.title}</h3>
            <div className="smartphonesmenu-links">
              {currentContent.items.map((item, index) => (
                <a key={index} href={item.url} className="smartphonesmenu-link">
                  {item.name}
                  {item.badge && <span className="smartphonesmenu-badge">{item.badge}</span>}
                </a>
              ))}
            </div>
          </div>
        ...
        {/* Промо-блоки */}
        <div className="smartphonesmenu-promo-section">
          <h3 className="smartphonesmenu-promo-section-title">Может заинтересовать вас</h3>
          <div className="smartphonesmenu-promo-row">
            {currentContent.promos.map((promo, index) => (
              <a key={index} href={`/promo/${promo.title.toLowerCase().replace(/\s+/g, '-')}`} className="smartphonesmenu-promo-card">
                <div className="smartphonesmenu-promo-banner">
                  <img 
                    src={promo.image}
                    alt={promo.alt}
                    className="smartphonesmenu-promo-image"
                  />
                </div>
              </a>
            ))}
