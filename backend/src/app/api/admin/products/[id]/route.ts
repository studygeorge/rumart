...
export async function PUT(
  ...
  try {
    ...
    // ✅ Фильтруем варианты - ДОБАВЛЕНЫ ПОЛЯ 'storage' И 'gpu'
    const allowedVariantFields = [
      'color', 'colorHex', 'memory', 'storage', 'connectivity', 'processor', 'gpu',
    ...
    const cleanedVariants = (variants || []).map((variant: any) => {
      ...
      console.log('📦 Очищенный вариант (обновление):', cleaned) // ✅ Для отладки
      
      return cleaned
    })

    console.log('🚀 Обновление продукта с вариантами:', cleanedVariants) // ✅ Для отладки
